...
export function acquireLock(agentId: number, objectBuffer: any) {
  ...
  const oldValue = Atomics.compareExchange(int32, 0, 0, agentId);

  return oldValue === 0;
}

/**
 *  Try to release a lock acquired by {@link acquireLock} or {@link acquireLockWait}
 *
 * @param agentId
 * @param sab
 */
export function releaseLock(agentId: number, objectBuffer: any) {
  ...
  const oldValue = Atomics.compareExchange(int32, 0, agentId, 0);
...
export function acquireLockWait(
  ...
  if (r === "not-equal") {
    if (Atomics.compareExchange(int32, 0, 0, agentId) === 0) {
      return "have-lock";
    } else {
      return "miss-lock";
    }
